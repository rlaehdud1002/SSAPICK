...
public class CampusService {
	...
	public List<CampusData.SearchResponse> searchCampusByDescription(String description) {
		return campusRepository.findByDescription(description)
			.stream()
			.map(CampusData.SearchResponse::fromEntity)
			.toList();
	}
	...
	public List<ProfileData.Search> searchProfileByCampusIds(List<Long> campusIds) {
		return campusRepository.findUserByCampusIds(campusIds)
			.stream()
			.map(User::getProfile)
	...
	public List<ProfileData.Search> searchProfileByCampusId(Long campusId) {
		return campusRepository.findUserByCampusId(campusId)
			.stream()
			.map(User::getProfile)
			.map(ProfileData.Search::fromEntity)
			.toList();
