...
class PickServiceTest extends UserSupport {
    ...
    void 로그인한_사용자가_받은_픽_조회_테스트() throws Exception {
        ...
        assertThat(searches.stream().map(PickData.Search::getSender)).map(ProfileData.Search::getNickname).containsExactly(null, null, null);
        assertThat(searches.stream().map(PickData.Search::getSender)).map(ProfileData.Search::getProfileImage).containsExactly(null, null, null);
    }
    ...
    void 로그인한_사용자가_보낸_픽_조회_테스트() throws Exception {
        ...
        assertThat(searches).hasSize(3);
        assertThat(searches.stream().map(PickData.Search::getId)).contains(1L, 2L, 3L);
        assertThat(searches.stream().map(PickData.Search::getReceiver)).map(ProfileData.Search::getNickname).containsExactly(receiver.getName(), receiver.getName(), receiver.getName());
        assertThat(searches.stream().map(PickData.Search::getReceiver)).map(ProfileData.Search::getProfileImage).containsExactly(receiver.getProfile().getProfileImage(), receiver.getProfile().getProfileImage(), receiver.getProfile().getProfileImage());
    }
