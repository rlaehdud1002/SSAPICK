...
const ModiUserAddInfo = () => {
    const [UserAddInfo, setUserAddInfo] = useRecoilState(userAddState)
    ...
    const { register, handleSubmit, setValue, formState: { errors } } = useForm<AddUserForm>({
        defaultValues: {
            mbti: UserAddInfo.mbti,
            class: UserAddInfo.classNum,
            major: UserAddInfo.major,
            birth: UserAddInfo.birth,
            town: UserAddInfo.location,
            hobby: UserAddInfo.interest,
        }
    });
        ...
        <div className="flex flex-col my-5">
            ...
            <div className="flex w-full flex-col justify-center items-center space-y-2">
                ...
                <InfoInput name="birth" title="생년월일" register={register("birth", {
                    ...
                    pattern: {
                        value: /^\d{4}-\d{2}-\d{2}$/, message: "YYYY-MM-DD 형식으로 입력해주세요."
                    }
                })} errors={errors} />
