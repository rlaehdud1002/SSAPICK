...
// 보낸 메시지 조회
export const getSendMessage = async (page: number, size: number): Promise<IPaging<IMessage[]>> => {
  const {
    data: { success, data },
...
export const deleteReceivedMessage = async (messageId: number): Promise<void> => {
  const response = await instance.delete<BaseResponse<IEmpty>>(`/message/${messageId}/receive`);
...
export const deleteSendMessage = async (messageId: number): Promise<void> => {
  const response = await instance.delete<BaseResponse<IEmpty>>(`/message/${messageId}/send`);
