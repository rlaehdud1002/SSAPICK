  ...
  return (
    ...
    >
      <div className="flex justify-between">
        ...
        <div className="flex items-center mr-2">
          <div className="flex flex-col items-center mx-1">
            ...
            <span className="luckiest_guy my-1">{formatNum(information.pickco)}</span>
          </div>
          <div className="flex flex-col items-center mx-1">
            <ProfilePickIcon width={30} height={30} />
            <span className="luckiest_guy my-1">{formatNum(information.pickCount)}</span>
          </div>
          <div className="flex flex-col items-center ml-1">
            <FriendIcon width={30} height={22} isDefault={false} />
            <div className="luckiest_guy mt-2">{formatNum(information.followingCount)}</div>
          </div>
        </div>
      </div>
      ...
      <div className="ml-5 my-6">
        ...
        <div className="my-1">
          {information?.gender === "F" ? <span>여자</span> : <span>남자</span>}• {age}세 •{" "}
          {information?.hints[5].content} • {information?.hints[9].content}
        </div>
      </div>
    </div>
  );
